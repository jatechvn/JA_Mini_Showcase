...
> **Note:** the `ask_permission` tool and `gh.<action>(...)` request format below describe a specific sandboxed/cloud agent environment. Confirm the tool actually exists in the current session before relying on it — if it doesn't, fall back to asking the user directly in chat before running a GitHub-mutating command.
